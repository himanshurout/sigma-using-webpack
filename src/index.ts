...
function setHoveredNode(node?: string) {
  if (node) {
    console.log("neighbors", graph.neighbors(node));
...
function showTimeWiseGraph(start, end) {
  var perHourCount = Math.round(batchJobs.length / 24);
  var startIndex = start * perHourCount;
  var endIndex = end * perHourCount;
  console.log("index", startIndex, endIndex);
  var filteredJobs = batchJobs.slice(startIndex, endIndex);
  showGraph(filteredJobs);
...
$(function () {
  $("#slider-range").slider({
    ...
    slide: function (event, ui) {
      ...
      showTimeWiseGraph(ui.values[0], ui.values[1])
    }
  });
